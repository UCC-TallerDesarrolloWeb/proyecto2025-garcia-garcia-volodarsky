  ...
  return (
    <>
      ...
      <form className="shop-filters" onSubmit={(e) => e.preventDefault()}>
        <input type="search" id="qShop" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar…" />
        <select id="catShop" value={cat} onChange={(e) => setCat(e.target.value)}>
          <option value="">Categoría</option>
          <option value="camiseta">Camiseta</option>
          <option value="gorra">Gorra</option>
        </select>
        <select id="talleShop" value={talle} onChange={(e) => setTalle(e.target.value)}>
        ...
        <select id="precioShop" value={precio} onChange={(e) => setPrecio(e.target.value)}>
          <option value="">Precio</option>
          <option value="1">Hasta $20.000</option>
          <option value="2">$20.001 – $40.000</option>
          <option value="3">Más de $40.000</option>
        </select>
        <select id="ordenShop" value={orden} onChange={(e) => setOrden(e.target.value)}>
        ...
        {visibles.map((prod) => (
          <div key={prod.id} className="prod-card">
            ...
            <div className="prod-img">
              {prod.image && <img src={resolveImage(prod.image) || ''} alt={prod.name} />}
            </div>
            ...
            <div className="prod-meta">
              <div className="prod-price">{prod.price ? '$ ' + prod.price.toLocaleString() : ''}</div>
              <div className="prod-actions">
                <button className="btn primary" onClick={() => addToCart(prod)}>Agregar</button>
              </div>
            </div>
          </div>
        ))}
      ...
      {selected && (
        <div className="modalProd">
          ...
          <button onClick={() => { addToCart(selected); cerrarModal() }}>
            Agregar y cerrar
          </button>
